from cowait.test import capture_task_output
...
def test_task_success():
    ...
    output = capture_task_output('cowait.test.tasks.utility', inputs={'return': 'everything ok'})
...
def test_task_error():
    ...
    output = capture_task_output('cowait.test.tasks.utility', inputs={'error': True})
    assert len(output.unique('id')) == 1
    assert output.has(type=TASK_INIT)
