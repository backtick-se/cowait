...
from .notebook_utils import file_to_json, to_code_string
...
class NotebookRunner(Task):
    async def run(self, path: str, **inputs):
        ...
        code.appendBlock(4, self.cells_to_code(cells))
        ...
        for cell in cells:
            ...
            if cell_type == 'code':
                ...
                if len(source_rows) >= 0:
                    code.appendBlock(0, self.code_from_source(source_rows))

        return code

    def code_from_source(self, source_rows):
        code = CodeBuilder()

        _, first_line = self.strip_indentation(source_rows[0])
        if first_line.startswith('%%'):
            command, *args = first_line[2:].split(' ', 1)
            new_code = self.transform_cell_magic(source_rows[1:], command, args)
            if new_code:
                code.appendBlock(0, new_code)
        else:
            for row in source_rows:
                indentation, line = self.strip_indentation(row)
                if line.startswith('global '):
                    ...
                    new_code = self.transform_line_magic(command, args)
                    if new_code:
    ...
    def strip_indentation(self, row):
        line = row.lstrip(' ')
        return len(row) - len(line), line

    def transform_line_magic(self, command: str, args: str):
    ...
    def transform_cell_magic(self, rows: list, command: str, args: str):
